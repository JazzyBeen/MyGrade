Отличная идея. Грамотный `README.md` — это лицо твоего проекта. Если ты выложишь код на GitHub для портфолио, рекрутеры часто смотрят **только** в Readme и на структуру папок, даже не открывая сам код.

Поскольку мы делаем серьезный рефакторинг, в Readme стоит сделать акцент на используемых технологиях и архитектуре.

Вот шаблон, который ты можешь скопировать. Я написал его на русском, но для международного GitHub обычно используют английский (если нужно, могу перевести).
